 ...
 *
 * ⚠️ 海运 22 步、陆运 16 步是两套流程，必须按父单的 transportMode 取对应那条线来比快慢
...
export async function syncParentStatusFromChildren(
  ...
  const flow: string[] =
    parent.transportMode === "land" ? SHIPMENT_STATUS_FLOW_LAND : SHIPMENT_STATUS_FLOW;
  ...
  const alive = children.map((c: any) => c.currentStatus).filter((s: string) => !dead.has(s));

  // 全部子单都已退回/取消：按固定优先级选，不能取 children[0] ——
  // 那个查询没有排序，同时存在退回和取消时结果会随数据库返回顺序变化。
  // 退回优先于取消：货真的退回来了，比「单子作废」更需要被看见。
  if (alive.length === 0) {
    const kinds = new Set<string>(children.map((c: any) => c.currentStatus));
    const first = kinds.has("returned") ? "returned" : "cancelled";
    if (first !== parent.currentStatus) {
      await tx.shipment.update({
        where: { id: parent.id },
        data: { currentStatus: first, updatedAt: new Date() },
      });
    }
    return first;
  }
  ...
  // 取流程上最靠前的那个 = 走得最慢的那个。
  // 流程表里查不到的状态（历史脏数据）按「最慢」处理：宁可显示得保守，也不要谎报已送达。
  let slowest = alive[0];
