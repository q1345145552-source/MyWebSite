    ...
    <RoleShell allowedRole="admin" title="财务结算系统">
      <section style={{ border: "1px solid var(--l-soft)", borderRadius: 12, padding: 16, background: "var(--white)", marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 8 }}>
          <input value={form.orderId} onChange={(e) => setForm((v) => ({ ...v, orderId: e.target.value }))} placeholder="订单号" style={{ border: "1px solid var(--l-strong)", borderRadius: 8, padding: "8px 10px" }} />
          <input value={form.clientReceivable} onChange={(e) => setForm((v) => ({ ...v, clientReceivable: e.target.value }))} placeholder="应收 AR" style={{ border: "1px solid var(--l-strong)", borderRadius: 8, padding: "8px 10px" }} />
          <input value={form.supplierPayable} onChange={(e) => setForm((v) => ({ ...v, supplierPayable: e.target.value }))} placeholder="应付 AP" style={{ border: "1px solid var(--l-strong)", borderRadius: 8, padding: "8px 10px" }} />
          <input value={form.taxFee} onChange={(e) => setForm((v) => ({ ...v, taxFee: e.target.value }))} placeholder="税费" style={{ border: "1px solid var(--l-strong)", borderRadius: 8, padding: "8px 10px" }} />
          <select value={form.currency} onChange={(e) => setForm((v) => ({ ...v, currency: e.target.value }))} style={{ border: "1px solid var(--l-strong)", borderRadius: 8, padding: "8px 10px" }}>
      ...
      <section style={{ border: "1px solid var(--l-soft)", borderRadius: 12, padding: 16, background: "var(--white)" }}>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {profitItems.map((item: any) => (
            <div key={`${item.orderId}-${item.updatedAt}`} style={{ border: "1px solid var(--l-cool)", borderRadius: 8, padding: 8, background: "var(--s-cool)" }}>
